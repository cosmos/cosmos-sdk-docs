...
The easiest way to send transactions is using the CLI, as we have seen in the previous page when [interacting with a node](./02-00-overview-app#using-the-cli). For example, running the following command
...
The preferred way to broadcast a transaction is to use gRPC, though using REST (via `gRPC-gateway`) or the Tendermint RPC is also posible. An overview of the differences between these methods is exposed [here](../../develop/advanced-concepts/06-grpc_rest.md). For this tutorial, we will only describe the gRPC method.
