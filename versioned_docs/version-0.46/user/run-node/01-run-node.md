...
State sync is the act in which a node syncs the latest or close to the latest state of a blockchain. This is useful for users who don't want to sync all the blocks in history. Read more in [CometBFT documentation](https://docs.cometbft.com/v0.34/01-tx-lifecycle.mdstate-sync).
...
1. As mentioned in https://docs.cometbft.com/v0.34/01-tx-lifecycle.mdstate-sync, one must set a height and hash in the config.toml along with a few rpc servers (the afromentioned link has instructions on how to do this).
